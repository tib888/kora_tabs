...
fn note_to_pitch(note: &str) -> Option<(Pitch, String)> {
    ...
    let (name, octave_str) =
        if note.len() >= 2 && (&main_part[1..2] == "b" || &main_part[1..2] == "#") {
    ...
    base.map(|base| {
        (
            Pitch((octave + 1) * 1200 + 100 * base + cents),
            name.to_string(),
        )
    })
}
...
fn draw_svg(
    ...
    svg_doc = svg_doc.add(
        SvgText::new(format!("Tuning: {}", tuning.name))
            .set("x", margin)
            .set("y", 55)
            .set("font-family", "Arial")
            .set("font-size", 12)
    ...
    for note in notes {
        ...
        if note.position > last_position {
            ...
            if current_x > (page_width - margin * 2.0) {
                ...
                svg_doc =
                    draw_kora_system(svg_doc, current_y_offset, measure_count, margin, page_width);
            }
        ...
        if let Some(string_info) = tuning.notes.iter().find(|&p| p.pitch == note.pitch) {
            let color = if string_info.right {
                "#FF0000"
            } else {
                "#0000FF"
            }; // Red (Right), Blue (Left)
            ...
            let rect_x = if string_info.right {
                margin + current_x
            } else {
...
fn draw_ascii(
    ...
    for note in notes {
        notes_by_pos
            .entry((note.position * 1000.0) as i64)
            .or_default()
            .push(note.clone());
    }
...
fn draw_musicxml(song_title: &str, notes: &[Note]) -> Result<String, Box<dyn std::error::Error>> {
    ...
    let mut notes_by_pos: std::collections::BTreeMap<i64, Vec<&Note>> =
...
fn parse_musicxml(source: &PathBuf) -> Result<(String, Vec<Note>), Box<dyn std::error::Error>> {
        ...
        for measure in part.children().filter(|n| n.has_tag_name("measure")) {
            for element in measure.children().filter(|n| n.is_element()) {
                match element.tag_name().name() {
                    "attributes" => {
                        if let Some(divs) =
                            element.descendants().find(|d| d.has_tag_name("divisions"))
                        {
                    ...
                    "note" => {
                        ...
                        if let Some(pitch_node) =
                            element.children().find(|n| n.has_tag_name("pitch"))
                        {
                            let step = pitch_node
                                .children()
                            ...
                            let alter = pitch_node
                                ...
                                .map(|n| n.text().unwrap())
                                .unwrap_or("0");
                            if let Some((base_pitch, _name)) =
                                note_to_pitch(&format!("{}{}", step, octave))
                            {
                                let alter_cents = alter
                                    .parse::<f32>()
                        ...
                        if !element.children().any(|c| c.has_tag_name("chord")) {
                            if let Some(duration) =
                                element.descendants().find(|d| d.has_tag_name("duration"))
                            {
                                current_pos_in_divisions +=
                                    duration.text().unwrap().parse().unwrap_or(0.0);
                            }
                        }
                    }
                    "backup" => {
    ...
    if notes.is_empty() {
        // Fallback for simple/malformed XML
...
fn find_closest_note_in_tuning(pitch: Pitch, tuning: &KoraTuning) -> Option<&StringInfo> {
    ...
    if let Some(closest_note) = tuning
        .notes
        .iter()
        .min_by_key(|&note| (pitch.0 - note.pitch.0).abs())
    {
...
fn transcribe_audio(
    source: &PathBuf,
    tuning: &KoraTuning,
) -> Result<(String, Vec<Note>), Box<dyn std::error::Error>> {
    ...
    let mut magnitude_history: std::collections::VecDeque<Vec<f32>> =
        std::collections::VecDeque::with_capacity(HISTORY_LENGTH);
    let mut peak_history: std::collections::VecDeque<Vec<Pitch>> =
        std::collections::VecDeque::with_capacity(PEAK_HISTORY_LENGTH);
    ...
    for (window_idx, window) in samples.windows(window_size).step_by(hop_size).enumerate() {
        ...
        for i in 1..(max_freq_index - 1) {
                ...
                && magnitudes[i] > magnitudes[i + 1]
            {
                let is_onset = if magnitude_history.len() == HISTORY_LENGTH {
                    let avg_prev_mag =
                        magnitude_history.iter().map(|m| m[i]).sum::<f32>() / HISTORY_LENGTH as f32;
                ...
                if *peak_idx > 0 && *peak_idx < num_bins {
                    ...
                    if x < IMG_WIDTH {
                        ...
                        img.put_pixel(
                            x,
                            window_idx as u32,
                            Rgb([color.0[2], color.0[2], color.0[2]]),
                        );
                    }
                }
            }
        ...
        let current_peaks: Vec<Pitch> = peaks
            .iter()
        ...
        for &peak in &current_peaks {
            if peak_history.len() >= PEAK_HISTORY_LENGTH {
                let stable_peak = peak_history
                    .iter()
                    .filter(|peak_set| peak_set.contains(&peak))
                    .count()
                    >= PEAK_HISTORY_LENGTH - 1;
                let new_peak = !peak_history
                    .front()
                    .map_or(false, |last_set| last_set.contains(&peak));
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (source, output, tuning_name, format_option, is_audio) = match args.command {
        Commands::Draw {
            ...
            tuning,
            format,
        } => (source, output, tuning, format, false),
        Commands::Transcribe {
            source,
            output,
            tuning,
            format,
        } => (source, output, tuning, format, true),
    };
    ...
    let config_str =
        fs::read_to_string("tuning.toml").map_err(|e| format!("TOML Load Error: {}", e))?;
    let all_tunings: HashMap<String, Tuning> =
        toml::from_str(&config_str).map_err(|e| format!("TOML Parse Error: {}", e))?;

    let tuning_config = all_tunings
        .get(&tuning_name)
        .ok_or_else(|| format!("Tuning '{}' not found in tuning.toml", tuning_name))?;

    let mut notes: Vec<StringInfo> = tuning_config
        .left
        .split(',')
        .enumerate()
        .map(|(idx, note)| {
